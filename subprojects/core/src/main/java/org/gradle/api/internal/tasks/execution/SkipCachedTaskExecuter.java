...
public class SkipCachedTaskExecuter implements TaskExecuter {
    ...
    public void execute(final TaskInternal task, TaskStateInternal state, TaskExecutionContext context) {
        ...
        boolean cacheable;
        try {
            List<TaskCachingDisabledReason> reasons = new ArrayList<TaskCachingDisabledReason>();
            cacheable = taskOutputs.isCacheEnabled(reasons);
            state.setCacheable(cacheable);
            if (!cacheable) {
                taskCachingReasonsListener.notCacheable(task, reasons);
            }
        } catch (Exception t) {
